...
const STORAGE_KEY = 'miley_wrong_questions_v56';
...
const INITIAL_SEED_DATA = [
  ...
  {
    "id": "q_chinese_polyphone_que_character_009",
    "examPeriod": "一段",
    "subject": "國文",
    "errorReason": "觀念不懂",
    "concept": "字音字義辨析：多音多義字「闕」之音義統整（ㄑㄩㄝˋ／ㄑㄩㄝ）",
    "uploadDate": "2026-09-08",
    "mondayDate": "2026-09-07",
    "mondayDates": [
      "2026-09-07"
    ...
    "weekLabel": "2026-09-07 (最新週次)",
    "isGuessedOrUnstable": true,
    "mistakeNote": "「闕」字音義速記法：1.【ㄑㄩㄝˋ（去聲）】：①計算詞或歌曲單位（一闕詞，通「闋」）；②帝王居所、宮闕（如宮闕、城闕、朝天闕）；③姓氏（闕先生）。2.【ㄑㄩㄝ（陰平/一聲）】：①過失、缺點（拾遺補闕）；②脫漏，同「缺」（闕文、殘闕、付之闕如）。故 (B) 完全正確！",
    "stem": "下列文句「闕」字的字音與字義說明，何者「完全正確」？\n\n○ (A) 這篇考卷中有幾處「闕」文，需要比對古籍補齊：音ㄑㄩㄝˋ，指帝王居所\n○ (B) 蘇軾寫下了名傳千古的〈水調歌頭〉一「闕」：音ㄑㄩㄝˋ，計算詞或歌曲的單位\n○ (C) 臣子應當勇於向君王進言，拾遺補「闕」：音ㄑㄩㄝˋ，同「缺」，指缺點或疏失\n○ (D) 岳飛〈滿江紅〉「待從頭收拾舊山河，朝天「闕」」：音ㄑㄩㄝ，指過失或疏漏",
    "answer": "(B) 蘇軾寫下了名傳千古的〈水調歌頭〉一「闕」：音ㄑㄩㄝˋ，計算詞或歌曲的單位",
    "diagramUrl": "",
    "solution": "1. **「闕」字音義深入剖析與各選項檢驗**：\n- **(A) 說明錯誤【字音與字義皆誤】**：\n  - 「闕」文：指文字脫漏、殘缺不全之處。此處「闕」同「缺」，字音應讀作 **ㄑㄩㄝ**（陰平/第一聲），字義為「脫漏、殘缺」，而非帝王居所。\n- **(B) 說明完全正確【正確答案 ✔】**：\n  - 一「闕」詞：作為計算歌曲或詞的量詞單位（現代規範正字多寫作「闋」）。字音讀作 **ㄑㄩㄝˋ**（去聲/第四聲），完全符合題意與用法。\n- **(C) 說明錯誤【字音錯誤】**：\n  - 拾遺補「闕」：指採錄遺漏、匡正補救君王或政事的過失缺失。此處「闕」字義確實為「過失、疏失、缺點」，但字音應讀作 **ㄑㄩㄝ**（陰平/第一聲），不可讀為去聲。\n- **(D) 說明錯誤【字音與字義皆誤】**：\n  - 朝天「闕」：出自岳飛〈滿江紅〉「待從頭收拾舊山河，朝天闕」。「天闕」泛指天子所居住的皇宮、宮廷門樓。字音應讀作 **ㄑㄩㄝˋ**（去聲/第四聲），字義為「帝王居所、宮闕」，並非疏漏之意。\n\n2. **「闕」字核心音義速查統整表**：\n| 讀音 | 聲調 | 核心字義 | 常見成語與例句 |\n| :--- | :--- | :--- | :--- |\n| **ㄑㄩㄝˋ** | 去聲 (四聲) | ❶ 計算詞或歌曲的單位（通「闋」）<br>❷ 泛指帝王居所、宮門、城門樓<br>❸ 姓氏 | ❶ 一**闕**詞、詞**闕**<br>❷ 宮**闕**、城**闕**、朝天**闕**、訪神仙於城**闕**<br>❸ **闕**先生 |\n| **ㄑㄩㄝ** | 陰平 (一聲) | ❶ 過失、缺點、政治疏漏<br>❷ 脫漏、欠缺（同「缺」） | ❶ 拾遺補**闕**、袞職有**闕**<br>❷ **闕**文、殘**闕**、付之**闕**如、多聞**闕**疑 |\n\n3. **結論**：音義完全正確者為 **(B)**。",
    "errorCount": 1,
    "ebbinghausStage": 1,
    "consecutiveMastered": 0,
    "isArchived": false,
