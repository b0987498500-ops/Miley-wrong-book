...
const STORAGE_KEY = 'miley_wrong_questions_v58';
...
const INITIAL_SEED_DATA = [
  ...
  },
  {
    ...
    "subject": "國文",
    "errorReason": "觀念不懂",
    "concept": "字形字音字義辨析：形近字「闋（門＋癸）」與多音字「闕（門＋欮）」之字形音義全面統整",
    "uploadDate": "2026-09-08",
    "mondayDate": "2026-09-07",
    "mondayDates": [
      "2026-09-07"
    ...
    "weekLabel": "2026-09-07 (最新週次)",
    "isGuessedOrUnstable": true,
    "mistakeNote": "【闋】與【闕】為兩個不同字形！\n1.【闋】（門部＋癸）：字音只有【ㄑㄩㄝˋ（去聲）】，釋義專指「計算歌曲或詞的單位」（如：一闋詞、詞闋）。\n2.【闕】（門部＋欮）：為多音多義字：\n  ①【ㄑㄩㄝˋ（去聲）】：❶ 泛指帝王居住的地方（宮闕、城闕、朝天闕）；❷ 姓（闕先生）。\n  ②【ㄑㄩㄝ（陰平/一聲）】：❶ 過失、缺點（拾遺補闕）；❷ 脫漏，同「缺」（闕文、殘闕、付之闕如）。\n(A) 闕文指脫漏，應讀ㄑㄩㄝ；(C) 拾遺補闕指補救缺失，應讀ㄑㄩㄝ；(D) 朝天闕指帝王宮闕，應作「闕」且讀ㄑㄩㄝˋ。故 (B) 完全正確！",
    "stem": "下列文句「闋」與「闕」字的字形、字音與字義說明，何者「完全正確」？\n\n○ (A) 這篇考卷中有幾處「闕」文，需要比對古籍補齊：字形從「欮」，音ㄑㄩㄝˋ，指帝王居所\n○ (B) 蘇軾寫下了名傳千古的〈水調歌頭〉一「闋」：字形從「癸」，音ㄑㄩㄝˋ，計算詞或歌曲的單位\n○ (C) 臣子應當勇於向君王進言，拾遺補「闕」：字形從「欮」，音ㄑㄩㄝˋ，同「缺」，指缺點或疏失\n○ (D) 岳飛〈滿江紅〉「待從頭收拾舊山河，朝天「闋」」：字形從「癸」，音ㄑㄩㄝ，指過失或疏漏",
    "answer": "(B) 蘇軾寫下了名傳千古的〈水調歌頭〉一「闋」：字形從「癸」，音ㄑㄩㄝˋ，計算詞或歌曲的單位",
    "diagramUrl": "",
    "solution": "1. **「闋（門＋癸）」與「闕（門＋欮）」形音義深入剖析與選項檢驗**：\n- **(A) 說明錯誤【字音與字義皆誤】**：\n  - 「闕」文：字形從「欮」，指文字脫漏、殘缺不全之處。此處「闕」同「缺」，字音應讀作 **ㄑㄩㄝ**（陰平/第一聲），字義為「脫漏、殘缺」，而非帝王居所。\n- **(B) 說明完全正確【正確答案 ✔】**：\n  - 一「闋」詞：字形為門部＋**癸**。字音讀作 **ㄑㄩㄝˋ**（去聲/第四聲），專門作為**計算歌曲或詞的單位（量詞）**，用法完全正確。\n- **(C) 說明錯誤【字音錯誤】**：\n  - 拾遺補「闕」：字形從「欮」，指採錄遺漏、匡正補救君王或政事的過失缺失。此處「闕」字義確實為「過失、疏失、缺點」，但字音應讀作 **ㄑㄩㄝ**（陰平/第一聲），不可讀為去聲。\n- **(D) 說明錯誤【字形、字音與字義皆誤】**：\n  - 朝天「闕」：出自岳飛〈滿江紅〉「待從頭收拾舊山河，朝天闕」。「天闕」泛指天子所居住的皇宮、宮廷門樓，字形應從「欮」作**「闕」**，字音讀作 **ㄑㄩㄝˋ**（去聲/第四聲），字義為「帝王居所、宮闕」，並非歌曲單位（闋），亦非疏漏。\n\n2. **「闋」與「闕」字形字音字義速查極限對照表**：\n| 字形 | 偏旁結構 | 讀音與聲調 | 核心釋義 | 常見例詞與成語 |\n| :--- | :--- | :--- | :--- | :--- |\n| **闋** | 門 ＋ **癸** | **ㄑㄩㄝˋ** (去聲) | **計算歌曲或詞的單位（量詞）** | 一**闋**詞、詞**闋**、歌**闋** |\n| **闕** | 門 ＋ **欮** | **ㄑㄩㄝˋ** (去聲) | ❶ **泛指帝王居住的地方（宮門、城門樓）**<br>❷ **姓氏** | ❶ 宮**闕**、城**闕**、朝天**闕**<br>❷ **闕**先生 |\n| **闕** | 門 ＋ **欮** | **ㄑㄩㄝ** (陰平/一聲) | ❶ **過失、缺點、疏失**<br>❷ **脫漏，同「缺」** | ❶ 拾遺補**闕**、袞職有**闕**<br>❷ **闕**文、殘**闕**、付之**闕**如、多聞**闕**疑 |\n\n3. **結論**：字形、字音與字義完全正確者為 **(B)**。",
    "errorCount": 1,
    "ebbinghausStage": 1,
    "consecutiveMastered": 0,
    "isArchived": false,
